        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Employe Add</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
		                		...
		                		<form action="{{ route('employe.store') }}" method="post" enctype="multipart/form-data">
				                    ...
				                    <div class="form-group">
				                       <label for="address">Gender:</label>
				                        ...
				                        <div class="icheck-danger d-inline">
					                        <input type="radio" name="gender" value="male" checked id="radioDanger1">
				                    ...
				                    <div class="form-group">
				                       <label for="email">Date of Birth:</label>
				                      	@error('date_birth')
                                            <span class="text-danger">{{ $message }}</span>
                                        @enderror
				                       <input type="date" class="form-control" id="date_birth" name="date_birth" placeholder="Enter date of birth" value="{{old('date_birth')}}">
				                    </div>
				                    <div class="form-group">
					                ...
					                <div class="form-group">
					                  ...
					                  <select class="select2" name="designation_id" multiple="multiple" data-placeholder="Select a Designation" style="width: 100%;">
					                    	@foreach($departments as $department)
					                			<option value="{{ $department->id }}">{{ $department->designations }}</option>
					               			@endforeach
					                  </select>
					                </div>
					                <div class="form-group">
				                       <label for="joing_date">Date Of Joining:</label>
				                      	@error('joing_date')
                                            <span class="text-danger">{{ $message }}</span>
                                        @enderror
				                       <input type="date" class="form-control" id="joing_date" name="joing_date" placeholder="Enter date of birth" value="{{old('joing_date')}}">
				                    </div>
				                    <div class="form-group">
				                       <label for="joing_salary">Joining Salary:</label>
				                       	@error('joing_salary')
                                            <span class="text-danger">{{ $message }}</span>
                                        @enderror
				                       <input type="number" class="form-control" name="joing_salary" id="joing_salary" placeholder="Enter Joining Salary" value="{{old('joing_salary')}}">
				                    </div>
				                    <div class="form-group">
				                       <label for="expense">Expense:</label>
				                       	@error('expense')
                                            <span class="text-danger">{{ $message }}</span>
                                        @enderror
				                       <input type="text" class="form-control" name="expense" id="expense" placeholder="Enter expense" value="{{old('expense')}}">
				                    </div>
			                		<div class="form-group">
				                    ...
				                    <div id="information-part" class="content" role="tabpanel" aria-labelledby="information-part-trigger">
				                    	<div class="form-group">
                                        	...
                                        	@enderror
					                        <div class="mb-2">
								             	<img id="showImage" class="rounded avatar-lg" src="{{ asset('uploads/default-50x50.jpg')}}" alt="No Image" width="100px" height="80px;">
								            </div>
